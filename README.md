...
* from the local directory, in the terminal, type and run `npm install firebase`, `npm install recoil`,
`npm install react-error-boundary` & `npm i -D react-router-dom`. When this id done, run `npm start`. 